...
世界书条目通过**两条通道**自动匹配：变量变更驱动（`match_worldbook.py`）和用户输入关键词扫描（`INPUT_MATCHES`）。匹配到的条目完整正文由 `round_prepare.py` 自动检索并写入回合上下文。`reference.md` 不进入对话上下文（保护 prompt cache），索引常驻，未自动匹配的话题 AI 按需 Grep。每轮至多额外检索 2-3 个条目。
...
# Claude Code 会自动完成：启动服务器 → 扫描素材 → 导入记忆 → 初始化状态 → 生成开场叙事
...
| **Monitor** | — | 已弃用。现由 server.py 内置 `/api/wait_pending` 长轮询替代，不再累积后台进程 |
