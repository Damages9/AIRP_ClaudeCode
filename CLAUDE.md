...
**素材解析完成后，立即从卡片数据自动生成 `{ROOT}/skills/styles/openings.json`**：
- 提取 `first_mes` 中 `<maintext>` 的内容作为开场正文（`<p>` 标签包裹）
- 提取 `first_mes` 中 `<Status_block>` 下的行动选项作为开场选项（emoji + 文本）
- 如有 `alternate_greetings`，每个额外生成一个 opening entry
- 以 UTF-8 编码写入 openings.json，格式：`[{"id": 0, "label": "首句截取", "content": "<p>HTML正文</p>", "options": ["😏 选项一", ...]}]`
- 此步骤确保每次切换卡片时开场白选项自动跟随卡变更新，不残留旧卡数据
...
- `reference.md`：从角色卡/worldbook/小说 txt 提取世界观规则和人物关系
- `feedback.md`：初始为空，仅记录 `NSFW 档位: {当前档位}`
- `user.md`：根据角色卡中 `{{user}}` 的描述提取（如有），否则留空待用户设定
- `story_plan.md`：初始为空模板，`next_plan_at: 第8轮`
